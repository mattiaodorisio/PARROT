/**
 * Adapted from https://github.com/NyaanNyaan/library/blob/master/data-structure/van-emde-boas-tree.hpp
 */
...
namespace vEB_tree_impl {
    ...
    struct vEB_tree_node {
        ...
        void remove(int key) {
            ...
            chd[pos].remove(mask(key));
            if (chd[pos].empty()) map.remove(pos);
            if (mn == key) {
                if (mx == key) {
                    mn = inf, mx = -1;
                } else {
        ...
        bool contains(int key) const {
            int pos = key >> shift;
            return chd[pos].contains(mask(key));
        }
    ...
    struct vEB_tree_node<LOG, typename std::enable_if<LOG == 1>::type> {
        ...
        inline void remove(int key) { map &= ~(1ULL << key); }
        inline bool contains(int key) const { return (map >> key) & 1; }
...
using veb_tree = vEB_tree_impl::vEB_tree_node<4>;
